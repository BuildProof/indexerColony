 ...
 */
export const parsePermissionedAction = (action) => {
...
const getSinglePermissionProofs = async (network, colony, domainId, role, customAddress) => {
    const permissionAddress = customAddress || (await colony.signer?.getAddress());
    ...
    const hasPermissionInDomain = await colony.hasUserRole(permissionAddress, domainId, role);
    ...
    const hasPermissionInAParentDomain = await colony.hasUserRole(permissionAddress, foundDomainId, role);
...
const getMultiPermissionProofs = async (network, colony, domainId, roles, customAddress) => {
    const proofs = await Promise.all(roles.map((role) => getSinglePermissionProofs(network, colony, domainId, role, customAddress)));
 ...
 */
export const getPermissionProofs = async (network, colony, domainId, roles, customAddress) => {
    if (Array.isArray(roles)) {
        if (roles.length === 1) {
            return getPermissionProofs(network, colony, domainId, roles[0], customAddress);
        }
