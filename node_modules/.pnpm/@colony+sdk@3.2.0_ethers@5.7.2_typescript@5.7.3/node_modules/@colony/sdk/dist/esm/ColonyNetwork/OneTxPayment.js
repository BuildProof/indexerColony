import { Extension, ColonyRole, Id, getContractVersion, getExtensionHash, getPermissionProofs, isExtensionCompatible, } from '@colony/core';
import { constants } from 'ethers';
import { extractEvent } from '../utils.js';
import { OneTxPayment__factory as OneTxPaymentFactory7, } from '../contracts/OneTxPayment/7/index.js';
import { OneTxPayment__factory as OneTxPaymentFactory8, } from '../contracts/OneTxPayment/8/index.js';
import { OneTxPayment__factory as OneTxPaymentFactory9, } from '../contracts/OneTxPayment/9/index.js';
...
export class OneTxPayment {
    ...
    static async connect(colony) {
        const address = await colony.colonyNetwork
            ...
            .getExtensionInstallation(getExtensionHash(OneTxPayment.extensionType), colony.address);
        ...
        if (!isExtensionCompatible(OneTxPayment.extensionType, version, colony.version)) {
        ...
        const Factory = OneTxPayment.supportedVersions.find((v) => v.version === version)?.factory;
        if (!Factory) {
    ...
    static getLatestSupportedVersion() {
        return OneTxPayment.supportedVersions[OneTxPayment.supportedVersions.length - 1]?.version;
    }
    /**
    ...
    pay(recipient, amount, teamId, tokenAddress) {
        ...
        return this.colony.createColonyTxCreator(this.oneTxPaymentContract, 'makePaymentFundedFromDomain', async () => {
            // Manual permission proofs are needed here
            const [extPermissionDomainId, extChildSkillIndex] = await getPermissionProofs(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), setTeamId, [ColonyRole.Administration, ColonyRole.Funding], this.oneTxPaymentContract.address);
            const [userPermissionDomainId, userChildSkillIndex] = await getPermissionProofs(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), setTeamId, [ColonyRole.Administration, ColonyRole.Funding]);
            return [
            ...
            ...extractEvent('OneTxPaymentMade', receipt),
        }));
    }
    /**
    ...
    upgrade(toVersion) {
        ...
        return this.colony.createColonyTxCreator(this.colony.getInternalColonyContract(), 'upgradeExtension', [getExtensionHash(Extension.OneTxPayment), version], async (receipt) => ({
