import { getPermissionProofs, getCreateMotionProofs, Id, } from '@colony/core';
import { extractEvent } from '../utils.js';
import { MetaTxCreator } from './MetaTxCreator.js';
/**
 ...
 */
export class ColonyTxCreator extends MetaTxCreator {
    colony;
    permissionConfig;
    constructor(config) {
        super(config);
    ...
    async getMotionTx(motionDomain) {
        if (!this.colony) {
        ...
        const { actionCid, key, value, branchMask, siblings } = await getCreateMotionProofs(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), this.colony.reputation, this.colony.ext.motions.getInternalVotingReputationContract(), motionDomain, altTarget, encodedAction);
    ...
    async getEncodedMotionTx(motionDomain) {
        if (!this.colony) {
            throw new Error('Motions can only be created on a Colony');
        }
        if (!this.colony.ext.motions) {
    ...
    async getMetaMotionTx(motionDomain) {
        const [encodedTransaction, address] = await this.getEncodedMotionTx(motionDomain);
        return this.sendMetaTransaction(encodedTransaction, address);
    }
    async getMetaMotionMinded(tx) {
    ...
    async getArgs() {
        ...
        if (this.permissionConfig) {
            ...
            const [permissionDomainId, childSkillIndex] = await getPermissionProofs(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), this.permissionConfig.domain, this.permissionConfig.roles, this.permissionConfig.address);
     ...
     */
    motion(motionDomain = Id.RootDomain) {
        return {
            send: async () => {
     ...
     * @returns A motion transaction that can be `send` or `mined` or `encode`d.
     */
    metaMotion(motionDomain = Id.RootDomain) {
        return {
            send: async () => {
