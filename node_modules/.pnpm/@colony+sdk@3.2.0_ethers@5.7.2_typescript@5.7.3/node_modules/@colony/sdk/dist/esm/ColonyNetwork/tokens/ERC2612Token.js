import { ERC2612TokenFactory, } from '@colony/tokens';
...
export class ERC2612Token extends ERC20Token {
    ...
    constructor(colonyNetwork, token) {
        ...
        if (typeof token == 'string') {
            this.tokenClient = ERC2612TokenFactory.connect(token, colonyNetwork.config.signerOrProvider);
        }
        else {
     ...
     */
    permit(amount, spender) {
        return this.colonyNetwork.createEip2612TxCreator(this.tokenClient, 'permit', async () => {
            const tokenLocking = await this.colonyNetwork.getTokenLocking();
            const spenderArg = spender || tokenLocking.address;
            return [spenderArg, amount];
        }, async (receipt) => ({
