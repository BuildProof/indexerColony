import { getTokenLockingClient } from '@colony/tokens';
import { extractEvent } from '../utils.js';
export class TokenLocking {
    address;
    colonyNetwork;
    tokenLockingContract;
    constructor(colonyNetwork, address) {
        this.address = address;
        this.colonyNetwork = colonyNetwork;
        this.tokenLockingContract = getTokenLockingClient(this.address, colonyNetwork.config.signerOrProvider);
    }
    /**
    ...
    deposit(tokenAddress, amount) {
        return this.colonyNetwork.createMetaTxCreator(this.tokenLockingContract, 'deposit(address,uint256,bool)', [tokenAddress, amount, false], async (receipt) => ({
            // eslint-disable-next-line max-len
            ...extractEvent('UserTokenDeposited', receipt),
        }));
    }
    /**
    ...
    withdraw(tokenAddress, amount) {
        return this.colonyNetwork.createMetaTxCreator(this.tokenLockingContract, 'withdraw(address,uint256,bool)', [tokenAddress, amount, false], async (receipt) => ({
            ...extractEvent('UserTokenWithdrawn', receipt),
        }));
    }
    /**
     ...
     */
    async getUserDeposit(tokenAddress, user) {
        const userLock = await this.tokenLockingContract.getUserLock(tokenAddress, user);
     ...
     */
    async getUserApproval(tokenAddress, user, obligator) {
