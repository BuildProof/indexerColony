...
exports.VotingReputation = exports.Vote = void 0;
const ethers_1 = require("ethers");
const core_1 = require("@colony/core");
const events_1 = require("@colony/events");
const index_js_1 = require("../contracts/IVotingReputation/7/index.js");
const index_js_2 = require("../contracts/IVotingReputation/8/index.js");
const index_js_3 = require("../contracts/IVotingReputation/9/index.js");
...
const REP_DIVISOR = ethers_1.BigNumber.from(10).pow(18);
/**
...
class VotingReputation {
    ...
    static connect(colony) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            const address = yield colony.colonyNetwork
                ...
                .getExtensionInstallation((0, core_1.getExtensionHash)(VotingReputation.extensionType), colony.address);
            if (address === AddressZero) {
                throw new Error(`${VotingReputation.extensionType} extension is not installed for this Colony`);
            }
            const version = (yield (0, core_1.getContractVersion)(address, colony.config.signerOrProvider));
            if (!(0, core_1.isExtensionCompatible)(VotingReputation.extensionType, version, colony.version)) {
                throw new Error(`Version ${version} of the ${VotingReputation.extensionType} contract is not compatible with the installed Colony contract version ${colony.version}`);
            }
            const Factory = (_a = VotingReputation.supportedVersions.find((v) => v.version === version)) === null || _a === void 0 ? void 0 : _a.factory;
            if (!Factory) {
                throw new Error(`Version ${version} of the ${VotingReputation.extensionType} contract is not supported in the SDK as of now`);
            }
    ...
    static getLatestSupportedVersion() {
        var _a;
        return (_a = VotingReputation.supportedVersions[VotingReputation.supportedVersions.length - 1]) === null || _a === void 0 ? void 0 : _a.version;
    }
    constructor(colony, votingReputationContract, deployedVersion) {
     ...
     */
    createMotionSalt(motionId) {
        return __awaiter(this, void 0, void 0, function* () {
            const { address } = this.votingReputationContract;
            const motionNumber = ethers_1.BigNumber.from(motionId).toNumber();
            /*
            ...
            const signature = yield this.colony
                .getInternalColonyContract()
                .signer.signMessage(message);
            return ethers_1.utils.keccak256(signature);
        });
    }
    /**
     ...
     */
    getSideVoted(motionId, salt, reputation) {
        return __awaiter(this, void 0, void 0, function* () {
            const { key, value, branchMask, siblings } = reputation;
            let sideVoted;
            try {
                yield this.votingReputationContract.estimateGas.revealVote(motionId, salt, 0, key, value, branchMask, siblings);
                sideVoted = Vote.Nay;
            }
            catch (nayErr) {
                if (nayErr.message.includes('voting-rep-secret-no-match')) {
                    try {
                        yield this.votingReputationContract.estimateGas.revealVote(motionId, salt, 1, key, value, branchMask, siblings);
                        sideVoted = Vote.Yay;
                    }
                    catch (yayErr) {
     ...
     */
    initialize(totalStakeFraction, voterRewardFraction, userMinStakeFraction, maxVoteFraction, stakePeriod, submitPeriod, revealPeriod, escalationPeriod) {
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'initialise', [
            totalStakeFraction,
            voterRewardFraction,
            ...
            return (Object.assign({}, (0, utils_js_1.extractEvent)('ExtensionInitialised', receipt)));
        }));
    }
    /**
    ...
    getMotion(motionId) {
        return __awaiter(this, void 0, void 0, function* () {
            const motionCount = yield this.votingReputationContract.getMotionCount();
     ...
     */
    getMotionResult(motionId) {
        return __awaiter(this, void 0, void 0, function* () {
            const state = yield this.getMotionState(motionId);
            if (state < core_1.MotionState.Finalizable) {
                return null;
            }
            const motion = yield this.getMotion(motionId);
            const totalStakeFraction = yield this.votingReputationContract.getTotalStakeFraction();
            const requiredStake = motion.skillRep
                .mul(totalStakeFraction)
                .div(REP_DIVISOR);
            if (motion.stakes[0].gte(requiredStake) &&
                motion.stakes[1].lt(requiredStake)) {
                return Vote.Nay;
            }
            if (motion.stakes[1].gte(requiredStake) &&
    ...
    getMinStake(motion, vote) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const userMinStakeFraction = yield this.votingReputationContract.getUserMinStakeFraction();
            // Total amount that is needed for activation
            const requiredStakeForActivation = ethers_1.BigNumber.from(skillRep)
                .mul(totalStakeFraction)
                .div(REP_DIVISOR);
            ...
            if (vote === Vote.Nay &&
                ethers_1.BigNumber.from(totalNay).lt(requiredStakeForActivation)) {
                const requiredNay = requiredStakeForActivation.sub(ethers_1.BigNumber.from(totalNay));
                minStake = requiredNay.lt(minStakePerUser)
                    ? requiredNay
                    : minStakePerUser;
            }
            else if (ethers_1.BigNumber.from(totalYay).lt(requiredStakeForActivation)) {
                const requiredYay = requiredStakeForActivation.sub(ethers_1.BigNumber.from(totalYay));
    ...
    getRemainingStakes(motionId) {
        return __awaiter(this, void 0, void 0, function* () {
            const { skillRep, stakes: [totalNay, totalYay], } = yield this.getMotion(motionId);
            // Fraction of the total amount that is required to be staked for either side
            // (Yay or Nay) to be activated
            const totalStakeFraction = yield this.votingReputationContract.getTotalStakeFraction();
            // Fraction of the total amount that has to be staked per transaction
     ...
     */
    createDecision(team = core_1.Id.RootDomain) {
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'createMotion', () => __awaiter(this, void 0, void 0, function* () {
            const { actionCid, key, value, branchMask, siblings } = yield (0, core_1.getCreateMotionProofs)(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), this.colony.reputation, this.votingReputationContract, team, AddressZero, core_1.DecisionMotionCode);
            return [
                team,
                actionCid,
                AddressZero,
                core_1.DecisionMotionCode,
                key,
                value,
                branchMask,
                siblings,
            ...
            return (Object.assign({}, (0, utils_js_1.extractEvent)('MotionCreated', receipt)));
        }));
    }
    /**
     * Annotate a decision with IPFS metadata to provide extra information
     ...
     */
    annotateDecision(txHash, metadata) {
        return this.colony.colonyNetwork.createMetaTxCreator(this.colony.getInternalColonyContract(), 'annotateTransaction', () => __awaiter(this, void 0, void 0, function* () {
            let cid;
            if (typeof metadata == 'string') {
                cid = metadata;
            }
            else {
                cid = yield this.colony.config.ipfs.uploadMetadata(events_1.MetadataType.Decision, metadata);
            }
            return [txHash, cid];
        }), (receipt) => __awaiter(this, void 0, void 0, function* () {
            return (Object.assign({}, (0, utils_js_1.extractEvent)('Annotation', receipt)));
        }), { metadataType: events_1.MetadataType.Decision });
    }
    /**
     ...
     */
    approveStake(amount, teamId = core_1.Id.RootDomain) {
        return this.colony.colonyNetwork.createMetaTxCreator(this.colony.getInternalColonyContract(), 'approveStake', [this.votingReputationContract.address, teamId, amount], (receipt) => __awaiter(this, void 0, void 0, function* () {
            const tokenLocking = yield this.colony.colonyNetwork.getTokenLocking();
            return Object.assign({}, (0, utils_js_1.extractCustomEvent)('UserTokenApproved', receipt, tokenLocking.getInternalTokenLockingContract().interface));
        }));
    }
    /**
     ...
     */
    stakeMotion(motionId, vote, amount) {
        const getArgs = () => __awaiter(this, void 0, void 0, function* () {
            const userAddress = yield this.colony.config.getSigner().getAddress();
            const motionState = yield this.votingReputationContract.getMotionState(motionId);
            if (motionState !== core_1.MotionState.Staking) {
                throw new Error(`Motion cannot be staked. It's currently in "${core_1.MotionState[motionState]}" state`);
            }
            const motion = yield this.getMotion(motionId);
            const tokenLocking = yield this.colony.colonyNetwork.getTokenLocking();
            const deposited = yield tokenLocking.getUserDeposit(this.colony.token.address, userAddress);
            if (deposited.lt(amount)) {
                throw new Error('Not enough tokens deposited for staking.');
            }
            const colonyApproval = yield tokenLocking.getUserApproval(this.colony.token.address, userAddress, this.colony.address);
            if (colonyApproval.lt(amount)) {
                throw new Error('Not enough tokens approved for staking in the Colony.');
            }
            const votingReputationApproval = yield this.colony
                .getInternalColonyContract()
                .getApproval(userAddress, this.address, motion.domainId);
            if (votingReputationApproval.lt(amount)) {
                throw new Error(`Not enough tokens approved for staking in the VotingReputation contract.`);
            }
            if (motion.events[0].mul(1000).lte(Date.now())) {
                throw new Error('The staking period for this Motion has passed already.');
            }
            const minStake = yield this.getMinStake(motion, vote);
            if (ethers_1.BigNumber.from(amount).lt(minStake)) {
                throw new Error(`The staked amount is too small. Please stake at least ${(0, core_1.toEth)(minStake)}`);
            }
            const { domainId, rootHash } = yield this.getMotion(motionId);
            const [permissionDomainId, childSkillIndex] = yield (0, core_1.getPermissionProofs)(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), domainId, core_1.ColonyRole.Arbitration, this.address);
            const { skillId } = yield this.colony.getTeam(domainId);
            const { key, value, branchMask, siblings } = yield this.colony.reputation.getReputationWithProofs(skillId, userAddress, rootHash);
            return [
                motionId,
                permissionDomainId,
                childSkillIndex,
                ethers_1.BigNumber.from(vote),
                amount,
                key,
                value,
                branchMask,
        ...
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'stakeMotion', getArgs, (receipt) => __awaiter(this, void 0, void 0, function* () {
            return (Object.assign(Object.assign({}, (0, utils_js_1.extractEvent)('MotionStaked', receipt)), (0, utils_js_1.extractEvent)('MotionEventSet', receipt)));
        }));
    }
    /**
     ...
     */
    submitVote(motionId, vote) {
        const getArgs = () => __awaiter(this, void 0, void 0, function* () {
            const motionState = yield this.votingReputationContract.getMotionState(motionId);
            if (motionState !== core_1.MotionState.Submit) {
                throw new Error(`Motion cannot be voted on at this time. It's currently in "${core_1.MotionState[motionState]}" state`);
            }
            const { domainId, rootHash } = yield this.getMotion(motionId);
            const { skillId } = yield this.colony.getTeam(domainId);
            const userAddress = yield this.colony.config.getSigner().getAddress();
            const { key, value, branchMask, siblings } = yield this.colony.reputation.getReputationWithProofs(skillId, userAddress, rootHash);
            const salt = yield this.createMotionSalt(motionId);
            const hash = ethers_1.utils.solidityKeccak256(['bytes', 'uint256'], [salt, vote]);
            return [motionId, hash, key, value, branchMask, siblings];
        });
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'submitVote', getArgs, (receipt) => __awaiter(this, void 0, void 0, function* () {
            return (Object.assign({}, (0, utils_js_1.extractEvent)('MotionVoteSubmitted', receipt)));
        }));
    }
    /**
     ...
     */
    revealVote(motionId, vote) {
        const getArgs = () => __awaiter(this, void 0, void 0, function* () {
            const motionState = yield this.votingReputationContract.getMotionState(motionId);
            if (motionState !== core_1.MotionState.Reveal) {
                throw new Error(`Motion cannot be revealed at this time. It's currently in "${core_1.MotionState[motionState]}" state`);
            }
            const { domainId, rootHash } = yield this.getMotion(motionId);
            const { skillId } = yield this.colony.getTeam(domainId);
            const userAddress = yield this.colony.config.getSigner().getAddress();
            const reputation = yield this.colony.reputation.getReputationWithProofs(skillId, userAddress, rootHash);
            const salt = yield this.createMotionSalt(motionId);
            const foundVote = vote || (yield this.getSideVoted(motionId, salt, reputation));
            if (!foundVote) {
                throw new Error(`Could not find a vote cast by ${userAddress} for motion ${motionId}`);
            }
            const { key, value, branchMask, siblings } = reputation;
            return [
                motionId,
                salt,
                ethers_1.BigNumber.from(foundVote),
                key,
                value,
                branchMask,
                siblings,
        ...
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'revealVote', getArgs, (receipt) => __awaiter(this, void 0, void 0, function* () {
            return (Object.assign({}, (0, utils_js_1.extractEvent)('MotionVoteRevealed', receipt)));
        }));
    }
    /**
     ...
     */
    escalateMotion(motionId, newTeamId) {
        const getArgs = () => __awaiter(this, void 0, void 0, function* () {
            const motionState = yield this.votingReputationContract.getMotionState(motionId);
            if (motionState !== core_1.MotionState.Closed) {
                throw new Error(`Motion cannot be escalated at this time. It's currently in "${core_1.MotionState[motionState]}" state`);
            }
            const { domainId, rootHash } = yield this.getMotion(motionId);
            const { skillId } = yield this.colony.getTeam(newTeamId);
            const userAddress = yield this.colony.config.getSigner().getAddress();
            const childIndex = yield (0, core_1.getChildIndex)(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), newTeamId, domainId);
            const { key, value, branchMask, siblings } = yield this.colony.reputation.getReputationWithProofs(skillId, userAddress, rootHash);
            return [
                motionId,
                newTeamId,
        ...
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'escalateMotion', getArgs, (receipt) => __awaiter(this, void 0, void 0, function* () {
            return (Object.assign({}, (0, utils_js_1.extractEvent)('MotionEscalated', receipt)));
        }));
    }
    /**
    ...
    finalizeMotion(motionId) {
        const getArgs = () => __awaiter(this, void 0, void 0, function* () {
            const motionState = yield this.votingReputationContract.getMotionState(motionId);
            if (motionState !== core_1.MotionState.Finalizable) {
                throw new Error(`Motion cannot be finalized. It's currently in "${core_1.MotionState[motionState]}" state`);
            }
            return [motionId];
        });
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'finalizeMotion', getArgs, (receipt) => __awaiter(this, void 0, void 0, function* () {
            return (Object.assign({}, (0, utils_js_1.extractEvent)('MotionFinalized', receipt)));
        }));
    }
    /**
    ...
    upgrade(toVersion) {
        ...
        return this.colony.createColonyTxCreator(this.colony.getInternalColonyContract(), 'upgradeExtension', [(0, core_1.getExtensionHash)(core_1.Extension.VotingReputation), version], (receipt) => __awaiter(this, void 0, void 0, function* () {
    ...
    getInitializationOptions() {
        return __awaiter(this, void 0, void 0, function* () {
            const promises = [
                this.votingReputationContract.getTotalStakeFraction(),
                this.votingReputationContract.getVoterRewardFraction(),
                this.votingReputationContract.getUserMinStakeFraction(),
                this.votingReputationContract.getMaxVoteFraction(),
                this.votingReputationContract.getStakePeriod(),
                this.votingReputationContract.getSubmitPeriod(),
                this.votingReputationContract.getRevealPeriod(),
                this.votingReputationContract.getEscalationPeriod(), // escalationPhaseDuration
            ];
            const [requiredStake, voterReward, minimumUserStake, endVoteThreshold, stakePhaseDuration, votingPhaseDuration, revealPhaseDuration, escalationPhaseDuration,] = yield Promise.all(promises);
            return {
                requiredStake,
                voterReward,
                minimumUserStake,
...
exports.VotingReputation = VotingReputation;
VotingReputation.supportedVersions = [
    { version: 7, factory: index_js_1.IVotingReputation__factory },
    { version: 8, factory: index_js_2.IVotingReputation__factory },
    { version: 9, factory: index_js_3.IVotingReputation__factory },
];
VotingReputation.extensionType = core_1.Extension.VotingReputation;
