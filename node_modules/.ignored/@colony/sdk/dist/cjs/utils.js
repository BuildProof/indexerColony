"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.extractCustomEvent = exports.extractEvent = void 0;
/** Extract event args from a contract receipt */
// TODO: Make it possible to retrieve multiple events of the same kind
const extractEvent = (eventName, receipt) => {
    if ('events' in receipt && receipt.events) {
        const event = receipt.events.find((ev) => ev.event === eventName);
        if (event === null || event === void 0 ? void 0 : event.args) {
            return event.args;
        }
    }
    else if ('parsedLogs' in receipt && receipt.parsedLogs) {
        const event = receipt.parsedLogs.find((ev) => ev.name === eventName);
        if (event === null || event === void 0 ? void 0 : event.args) {
            return event.args;
        }
    }
    return undefined;
};
exports.extractEvent = extractEvent;
/** Manually extract an event using the interface (e.g. if emitting contract is a different one than the calling contract) */
const extractCustomEvent = (eventName, receipt, iface) => {
    let events;
    if ('events' in receipt && receipt.events) {
        events = receipt.events;
    }
    else if ('logs' in receipt && receipt.logs) {
        events = receipt.logs;
    }
    else {
        events = [];
    }
    for (let i = 0; i < events.length; i += 1) {
        try {
            return iface.decodeEventLog(eventName, events[i].data, events[i].topics);
        }
        catch (e) {
            // ignore
        }
    }
    return undefined;
};
exports.extractCustomEvent = extractCustomEvent;
