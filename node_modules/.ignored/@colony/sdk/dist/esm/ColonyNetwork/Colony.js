...
import { Id, ColonyRole, ReputationClient, getChildIndex, getContractVersion, getExtensionHash, getPermissionProofs, isExtensionCompatible, hex2ColonyRoles, colonyRoles2Hex, } from '@colony/core';
import { MetadataType, } from '@colony/events';
import { IColony__factory as ColonyFactory11, } from '../contracts/IColony/11/index.js';
import { IColony__factory as ColonyFactory12, } from '../contracts/IColony/12/index.js';
import { IColony__factory as ColonyFactory13, } from '../contracts/IColony/13/index.js';
import { ColonyTxCreator, } from '../TxCreator/index.js';
import { extractEvent, extractCustomEvent } from '../utils.js';
import { OneTxPayment } from './OneTxPayment.js';
import { ERC20Token, getToken } from './tokens/index.js';
import { VotingReputation } from './VotingReputation.js';
/** Extensions that are supported by Colony SDK */
export var SupportedExtension;
...
export class Colony {
     ...
     */
    static async connect(colonyNetwork, address) {
        const version = (await getContractVersion(address, colonyNetwork.config.signerOrProvider));
        const Factory = Colony.supportedVersions.find((v) => v.version === version)?.factory;
        if (!Factory) {
    ...
    constructor(colonyNetwork, colony, token, deployedVersion) {
        ...
        this.ext = {};
        this.reputation = new ReputationClient(colonyNetwork.getInternalNetworkContract(), colony, { customEndpointUrl: colonyNetwork.config.reputationOracleEndpoint });
        this.token = token;
     ...
     */
    async getBalance(tokenAddress, teamId) {
        if (teamId) {
     ...
     */
    edit(metadata) {
        return this.createColonyTxCreator(this.colony, 'editColony', async () => {
            let cid;
            if (typeof metadata == 'string') {
                cid = metadata;
            }
            else {
                cid = await this.config.ipfs.uploadMetadata(MetadataType.Colony, metadata);
            }
            return [cid];
        }, async (receipt) => ({
            ...extractEvent('ColonyMetadata', receipt),
        }), {
    ...
    createTeam(metadata) {
        if (!metadata) {
            return this.createPermissionedColonyTxCreator(this.colony, 'addDomain(uint256,uint256,uint256)', [Id.RootDomain], {
                roles: ColonyRole.Architecture,
                domain: Id.RootDomain,
            }, async (receipt) => ({
                ...extractEvent('DomainAdded', receipt),
                ...extractEvent('FundingPotAdded', receipt),
            }));
        }
        return this.createPermissionedColonyTxCreator(this.colony, 'addDomain(uint256,uint256,uint256,string)', async () => {
            let cid;
            if (typeof metadata == 'string') {
                cid = metadata;
            }
            else {
                cid = await this.config.ipfs.uploadMetadata(MetadataType.Domain, metadata);
            }
            return [Id.RootDomain, cid];
        }, {
            roles: ColonyRole.Architecture,
            domain: Id.RootDomain,
        }, async (receipt) => ({
            ...extractEvent('DomainAdded', receipt),
            ...extractEvent('FundingPotAdded', receipt),
            ...extractEvent('DomainMetadata', receipt),
     ...
     */
    editTeam(metadata) {
        return this.createPermissionedColonyTxCreator(this.colony, 'editDomain', async () => {
            let cid;
            if (typeof metadata == 'string') {
                cid = metadata;
            }
    ...
    deprecateTeam(teamId, deprecated) {
        return this.createPermissionedColonyTxCreator(this.colony, 'deprecateDomain', [teamId, deprecated], {
            ...
            ...extractEvent('DomainDeprecated', receipt),
        }));
    }
    /**
    ...
    claimFunds(tokenAddress) {
        const token = tokenAddress || this.token.address;
        return this.createColonyTxCreator(this.colony, 'claimColonyFunds', [token], async (receipt) => ({
            // eslint-disable-next-line max-len
            ...extractEvent('ColonyFundsClaimed', receipt),
        }));
    }
    /**
    ...
    moveFundsToTeam(amount, toTeam, fromTeam, tokenAddress) {
        ...
        // eslint-disable-next-line max-len
        'moveFundsBetweenPots(uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,address)', async () => {
            const domain = await this.colony.getDomain(toTeam);
            if (domain.fundingPotId.isZero()) {
                throw new Error(`Team with id ${BigNumber.from(toTeam).toString()} does not exist`);
            }
            // Manual permission proofs are needed here
            const [permissionDomainId, childSkillIndex] = await getPermissionProofs(this.colonyNetwork.getInternalNetworkContract(), this.colony, parentTeam, ColonyRole.Funding);
            const networkClient = this.colonyNetwork.getInternalNetworkContract();
            const fromChildSkillIndex = await getChildIndex(networkClient, this.colony, parentTeam, setFromTeam);
            const toChildSkillIndex = await getChildIndex(networkClient, this.colony, parentTeam, toTeam);
            const { fundingPotId: fromPot } = await this.colony.getDomain(setFromTeam);
            ...
            // eslint-disable-next-line max-len
            ...extractEvent('ColonyFundsMovedBetweenFundingPots', receipt),
        }));
    }
    /**
     ...
     */
    async getReputation(userAddress, teamId = Id.RootDomain, decimalPoints = 3) {
     ...
     */
    async getReputationPoints(userAddress, teamId = Id.RootDomain) {
        const { skillId } = await this.colony.getDomain(teamId);
        const { reputationAmount } = await this.reputation.getReputation(skillId, userAddress);
     ...
     */
    async getReputationAcrossTeams(userAddress) {
    ...
    makeArbitraryTransaction(target, action) {
        return this.createColonyTxCreator(this.colony, 'makeArbitraryTransactions', [[target], [action], false], async (receipt) => ({
            // eslint-disable-next-line max-len
            ...extractEvent('ArbitraryTransaction(address,address,bytes,bool)', receipt),
        }));
    }
    /**
    ...
    annotateTransaction(txHash, metadata) {
        return this.createColonyTxCreator(this.colony, 'annotateTransaction', async () => {
            let cid;
            if (typeof metadata == 'string') {
                cid = metadata;
            }
            else {
                cid = await this.config.ipfs.uploadMetadata(MetadataType.Annotation, metadata);
            }
            return [txHash, cid];
        }, async (receipt) => ({
    ...
    installExtension(extension) {
        ...
        const networkClient = this.colonyNetwork.getInternalNetworkContract();
        return this.createColonyTxCreator(this.colony, 'installExtension', [getExtensionHash(extensionType), Extension.getLatestSupportedVersion()], async (receipt) => ({
            // eslint-disable-next-line max-len
            ...extractCustomEvent('ExtensionInstalled', receipt, networkClient.interface),
        }));
    }
    /**
     ...
     */
    async getRoles(address, teamId = Id.RootDomain) {
     ...
     */
    setRoles(address, roles, teamId = Id.RootDomain) {
        return this.createPermissionedColonyTxCreator(this.colony, 'setUserRoles', async () => {
            ...
            // eslint-disable-next-line max-len
            ...extractEvent('ColonyRoleSet', receipt),
            ...extractEvent('RecoveryRoleSet', receipt),
        }));
    }
    /**
     ...
     * | `setTo` | number | Whether the role was awarded or removed |
     */
    unsetRoles(address, roles, teamId = Id.RootDomain) {
        return this.createPermissionedColonyTxCreator(this.colony, 'setUserRoles', async () => {
            const oldRoles = await this.getRoles(address, teamId);
     ...
     */
    mint(amount) {
        return this.createColonyTxCreator(this.colony, 'mintTokens', [amount], async (receipt) => ({
            ...extractEvent('TokensMinted', receipt),
        }));
    }
    /**
    ...
    deployTokenAuthority(allowedToTransfer) {
        const networkClient = this.colonyNetwork.getInternalNetworkContract();
        return this.colonyNetwork.createMetaTxCreator(networkClient, 'deployTokenAuthority', async () => {
            let allowed = [];
     ...
     */
    enterRecoveryMode() {
        return this.createColonyTxCreator(this.colony, 'enterRecoveryMode', [], async (receipt) => ({
            ...extractEvent('RecoveryModeEntered', receipt),
        }));
    }
    /**
     * Exit Recovery Mode
     ...
     */
    exitRecoveryMode() {
        return this.createColonyTxCreator(this.colony, 'exitRecoveryMode', [], async (receipt) => ({
            ...extractEvent('RecoveryModeExited', receipt),
        }));
    }
    /**
    ...
    upgrade(toVersion) {
        ...
        return this.createColonyTxCreator(this.colony, 'upgrade', [version], async (receipt) => ({
            // eslint-disable-next-line max-len
            ...extractEvent('ColonyUpgraded', receipt),
        }));
    }
    /**
     ...
     */
    awardReputation(address, amount, team = Id.RootDomain) {
        if (BigNumber.from(amount).lte(0)) {
            throw new Error('Reputation award must be bigger than 0');
        }
        return this.createColonyTxCreator(this.colony, 'emitDomainReputationReward', [team, address, amount], async (receipt) => ({
            ...extractEvent('ArbitraryReputationUpdate', receipt),
        }));
    }
    /**
     ...
     */
    smiteReputation(address, amount, team = Id.RootDomain) {
        if (BigNumber.from(amount).lte(0)) {
            throw new Error('Reputation smite must be bigger than 0');
        }
        return this.createPermissionedColonyTxCreator(this.colony, 'emitDomainReputationPenalty', [team, address, BigNumber.from(0).sub(amount)], {
            domain: team,
