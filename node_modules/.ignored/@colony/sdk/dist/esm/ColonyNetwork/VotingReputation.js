...
import { Extension, Id, MotionState, DecisionMotionCode, ColonyRole, getChildIndex, getContractVersion, getCreateMotionProofs, getExtensionHash, getPermissionProofs, isExtensionCompatible, toEth, } from '@colony/core';
import { MetadataType, } from '@colony/events';
import { IVotingReputation__factory as VotingReputationFactory7, } from '../contracts/IVotingReputation/7/index.js';
import { IVotingReputation__factory as VotingReputationFactory8, } from '../contracts/IVotingReputation/8/index.js';
import { IVotingReputation__factory as VotingReputationFactory9, } from '../contracts/IVotingReputation/9/index.js';
...
export class VotingReputation {
    ...
    static extensionType = Extension.VotingReputation;
    // TODO: This could be abstracted in the future
    static async connect(colony) {
        const address = await colony.colonyNetwork
            .getInternalNetworkContract()
            .getExtensionInstallation(getExtensionHash(VotingReputation.extensionType), colony.address);
        if (address === AddressZero) {
            throw new Error(`${VotingReputation.extensionType} extension is not installed for this Colony`);
        }
        const version = (await getContractVersion(address, colony.config.signerOrProvider));
        if (!isExtensionCompatible(VotingReputation.extensionType, version, colony.version)) {
        ...
        const Factory = VotingReputation.supportedVersions.find((v) => v.version === version)?.factory;
        if (!Factory) {
    ...
    static getLatestSupportedVersion() {
        return VotingReputation.supportedVersions[VotingReputation.supportedVersions.length - 1]?.version;
    }
     ...
     */
    async createMotionSalt(motionId) {
     ...
     */
    async getSideVoted(motionId, salt, reputation) {
        const { key, value, branchMask, siblings } = reputation;
        let sideVoted;
        try {
            await this.votingReputationContract.estimateGas.revealVote(motionId, salt, 0, key, value, branchMask, siblings);
            sideVoted = Vote.Nay;
        }
        catch (nayErr) {
            if (nayErr.message.includes('voting-rep-secret-no-match')) {
                try {
                    await this.votingReputationContract.estimateGas.revealVote(motionId, salt, 1, key, value, branchMask, siblings);
                    sideVoted = Vote.Yay;
                }
                catch (yayErr) {
    ...
    initialize(totalStakeFraction, voterRewardFraction, userMinStakeFraction, maxVoteFraction, stakePeriod, submitPeriod, revealPeriod, escalationPeriod) {
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'initialise', [
            ...
            ...extractEvent('ExtensionInitialised', receipt),
        }));
    }
    /**
    ...
    async getMinStake(motion, vote) {
        ...
        const userMinStakeFraction = await this.votingReputationContract.getUserMinStakeFraction();
        ...
        if (vote === Vote.Nay &&
            ...
            const requiredNay = requiredStakeForActivation.sub(BigNumber.from(totalNay));
            minStake = requiredNay.lt(minStakePerUser)
                ? requiredNay
                : minStakePerUser;
        }
        else if (BigNumber.from(totalYay).lt(requiredStakeForActivation)) {
            const requiredYay = requiredStakeForActivation.sub(BigNumber.from(totalYay));
     ...
     */
    async getRemainingStakes(motionId) {
        const { skillRep, stakes: [totalNay, totalYay], } = await this.getMotion(motionId);
        // Fraction of the total amount that is required to be staked for either side
        // (Yay or Nay) to be activated
        const totalStakeFraction = await this.votingReputationContract.getTotalStakeFraction();
        // Fraction of the total amount that has to be staked per transaction
     ...
     */
    createDecision(team = Id.RootDomain) {
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'createMotion', async () => {
            const { actionCid, key, value, branchMask, siblings } = await getCreateMotionProofs(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), this.colony.reputation, this.votingReputationContract, team, AddressZero, DecisionMotionCode);
            return [
                team,
                actionCid,
                AddressZero,
            ...
            ...extractEvent('MotionCreated', receipt),
        }));
    }
    /**
     * Annotate a decision with IPFS metadata to provide extra information
    ...
    annotateDecision(txHash, metadata) {
        return this.colony.colonyNetwork.createMetaTxCreator(this.colony.getInternalColonyContract(), 'annotateTransaction', async () => {
            let cid;
            if (typeof metadata == 'string') {
                cid = metadata;
            }
            else {
                cid = await this.colony.config.ipfs.uploadMetadata(MetadataType.Decision, metadata);
            }
            return [txHash, cid];
        }, async (receipt) => ({
            ...extractEvent('Annotation', receipt),
     ...
     */
    approveStake(amount, teamId = Id.RootDomain) {
        return this.colony.colonyNetwork.createMetaTxCreator(this.colony.getInternalColonyContract(), 'approveStake', [this.votingReputationContract.address, teamId, amount], async (receipt) => {
            const tokenLocking = await this.colony.colonyNetwork.getTokenLocking();
            return {
                // eslint-disable-next-line max-len
                ...extractCustomEvent('UserTokenApproved', receipt, tokenLocking.getInternalTokenLockingContract().interface),
            };
        });
    }
    /**
    ...
    stakeMotion(motionId, vote, amount) {
        const getArgs = async () => {
            ...
            const motionState = await this.votingReputationContract.getMotionState(motionId);
            if (motionState !== MotionState.Staking) {
                throw new Error(`Motion cannot be staked. It's currently in "${MotionState[motionState]}" state`);
            }
            const motion = await this.getMotion(motionId);
            const tokenLocking = await this.colony.colonyNetwork.getTokenLocking();
            const deposited = await tokenLocking.getUserDeposit(this.colony.token.address, userAddress);
            if (deposited.lt(amount)) {
                throw new Error('Not enough tokens deposited for staking.');
            }
            const colonyApproval = await tokenLocking.getUserApproval(this.colony.token.address, userAddress, this.colony.address);
            ...
            if (BigNumber.from(amount).lt(minStake)) {
                throw new Error(`The staked amount is too small. Please stake at least ${toEth(minStake)}`);
            }
            const { domainId, rootHash } = await this.getMotion(motionId);
            const [permissionDomainId, childSkillIndex] = await getPermissionProofs(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), domainId, ColonyRole.Arbitration, this.address);
            const { skillId } = await this.colony.getTeam(domainId);
            const { key, value, branchMask, siblings } = await this.colony.reputation.getReputationWithProofs(skillId, userAddress, rootHash);
            return [
                motionId,
                permissionDomainId,
                childSkillIndex,
        ...
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'stakeMotion', getArgs, async (receipt) => ({
            ...extractEvent('MotionStaked', receipt),
            ...extractEvent('MotionEventSet', receipt),
        }));
    }
    /**
    ...
    submitVote(motionId, vote) {
        const getArgs = async () => {
            ...
            if (motionState !== MotionState.Submit) {
                throw new Error(`Motion cannot be voted on at this time. It's currently in "${MotionState[motionState]}" state`);
            }
            const { domainId, rootHash } = await this.getMotion(motionId);
            const { skillId } = await this.colony.getTeam(domainId);
            const userAddress = await this.colony.config.getSigner().getAddress();
        ...
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'submitVote', getArgs, async (receipt) => ({
            ...extractEvent('MotionVoteSubmitted', receipt),
        }));
    }
    /**
    ...
    revealVote(motionId, vote) {
        const getArgs = async () => {
            ...
            if (motionState !== MotionState.Reveal) {
                throw new Error(`Motion cannot be revealed at this time. It's currently in "${MotionState[motionState]}" state`);
            }
            const { domainId, rootHash } = await this.getMotion(motionId);
            const { skillId } = await this.colony.getTeam(domainId);
            const userAddress = await this.colony.config.getSigner().getAddress();
            const reputation = await this.colony.reputation.getReputationWithProofs(skillId, userAddress, rootHash);
            const salt = await this.createMotionSalt(motionId);
            const foundVote = vote || (await this.getSideVoted(motionId, salt, reputation));
            if (!foundVote) {
        ...
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'revealVote', getArgs, async (receipt) => ({
            ...extractEvent('MotionVoteRevealed', receipt),
        }));
    }
    /**
    ...
    escalateMotion(motionId, newTeamId) {
        const getArgs = async () => {
            ...
            if (motionState !== MotionState.Closed) {
                throw new Error(`Motion cannot be escalated at this time. It's currently in "${MotionState[motionState]}" state`);
            }
            const { domainId, rootHash } = await this.getMotion(motionId);
            const { skillId } = await this.colony.getTeam(newTeamId);
            const userAddress = await this.colony.config.getSigner().getAddress();
            const childIndex = await getChildIndex(this.colony.colonyNetwork.getInternalNetworkContract(), this.colony.getInternalColonyContract(), newTeamId, domainId);
            const { key, value, branchMask, siblings } = await this.colony.reputation.getReputationWithProofs(skillId, userAddress, rootHash);
            return [
                motionId,
                newTeamId,
        ...
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'escalateMotion', getArgs, async (receipt) => ({
            ...extractEvent('MotionEscalated', receipt),
        }));
    }
    /**
    ...
    finalizeMotion(motionId) {
        const getArgs = async () => {
            ...
            if (motionState !== MotionState.Finalizable) {
                throw new Error(`Motion cannot be finalized. It's currently in "${MotionState[motionState]}" state`);
            }
            return [motionId];
        };
        return this.colony.colonyNetwork.createMetaTxCreator(this.votingReputationContract, 'finalizeMotion', getArgs, async (receipt) => ({
            ...extractEvent('MotionFinalized', receipt),
        }));
    }
    /**
    ...
    upgrade(toVersion) {
        ...
        return this.colony.createColonyTxCreator(this.colony.getInternalColonyContract(), 'upgradeExtension', [getExtensionHash(Extension.VotingReputation), version], async (receipt) => ({
            ...extractEvent('ExtensionUpgraded', receipt),
        }));
    }
    /**
    ...
    async getInitializationOptions() {
        ...
        const [requiredStake, voterReward, minimumUserStake, endVoteThreshold, stakePhaseDuration, votingPhaseDuration, revealPhaseDuration, escalationPhaseDuration,] = await Promise.all(promises);
        return {
            requiredStake,
            voterReward,
            minimumUserStake,
