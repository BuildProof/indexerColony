...
import { addressesAreEqual, nonNullable, } from '@colony/core';
import { IpfsMetadata, } from './ipfs/index.js';
...
export class ColonyEventManager {
        ...
        return logs
            ...
            if (IpfsMetadata.eventSupportsMetadata(eventName)) {
                return {
                    ...
                    getMetadata: async () => {
                        const result = await this.ipfs.getMetadataForEvent(eventName, data.metadata);
                        return result;
                    },
                };
            }
     ...
     * @returns An array of {@link ColonyEvent}s
     */
    async getMultiEvents(filters, options = {}) {
        const filterArray = [].concat(filters);
        // Unique list of addresses
        const addresses = Array.from(new Set(filterArray.flatMap(({ address }) => address))).filter(nonNullable);
