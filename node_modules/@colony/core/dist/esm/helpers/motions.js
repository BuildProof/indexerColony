...
import { MotionTarget__factory as MotionTargetFactory, } from '../contracts/index.js';
import { parsePermissionedAction } from './permissions.js';
import { hex2ColonyRoles } from './network.js';
const { MaxUint256 } = constants;
const { AddressZero } = constants;
async function getCapabilityRolesAsArray(contract, _sig) {
    let rolesHexString;
    try {
        // This will work if the target has the getCapabilityRoles method
        rolesHexString = await contract.getCapabilityRoles(utils.hexZeroPad(_sig, 4));
 ...
 */
export async function getCreateMotionProofs(network, colony, reputation, votingReputation, domainId, altTarget, action) {
    let actionCid;
    const { skillId } = await colony.getDomain(domainId);
    const { sig, permissionDomainId, childSkillIndex } = parsePermissionedAction(action);
    const motionTarget = MotionTargetFactory.connect(altTarget, votingReputation.signer || votingReputation.provider);
    const capabilityRoles = await getCapabilityRolesAsArray(motionTarget, sig);
    // Requires root or is not a permissioned function
    if (capabilityRoles.includes(ColonyRole.Root) ||
        capabilityRoles.length === 0) {
        if (!BigNumber.from(domainId).eq(1)) {
            throw new Error(`On root or not permissioned actions, the domainId needs to be ${Id.RootDomain}`);
        }
        ...
        const { skillId: permissionSkillId } = await colony.getDomain(permissionDomainId);
        // Get the associated skill of the domain the action is taking place in
        const actionSkillId = await network.getChildSkillId(permissionSkillId, childSkillIndex);
    ...
    const { key, value, branchMask, siblings } = await reputation.getReputationWithProofs(skillId, AddressZero);
    return {
        actionCid,
        key,
        value,
