...
export class ReputationClient {
    static async fetchReputation(url) {
    ...
    async getRootHash(customRootHash) {
        const rootHash = customRootHash || (await this.network.getReputationRootHash());
        if (BigNumber.from(rootHash).isZero()) {
            throw new Error('No reputation for given rootHash found (yet)');
        }
        return rootHash;
    }
    async getBaseEndpoint(customRootHash) {
     ...
     */
    async getReputation(skillId, address, customRootHash) {
        ...
        const result = await ReputationClient.fetchReputation(url);
        return {
            ...result,
     ...
     * @returns Reputation data
     */
    async getReputationWithProofs(skillId, address, customRootHash) {
        const baseEndpoint = await this.getBaseEndpoint(customRootHash);
        const skillIdString = BigNumber.from(skillId || 0).toString();
     ...
     * @returns Reputation data
     */
    async getReputationAcrossDomains(address, customRootHash) {
        const baseEndpoint = await this.getBaseEndpoint(customRootHash);
        const url = `${baseEndpoint}/${address}/all`;
        const result = await ReputationClient.fetchReputation(url);
        const domainCount = await this.colony.getDomainCount();
        const allColonyDomains = (await Promise.all(Array.from(new Array(domainCount.toNumber())).map(async (_, index) => {
        ...
        return allColonyDomains.map((domain) => {
            ...
            const skillAssignedToDomain = (result?.reputations || []).find(({ skill_id: skillId }) => skillId === domain.skillId);
     ...
     * @returns Reputation data
     */
    async getMembersReputation(skillId, customRootHash) {
        const baseEndpoint = await this.getBaseEndpoint(customRootHash);
        const skillIdString = BigNumber.from(skillId || 0).toString();
     ...
     * @returns Reputation data
     */
    async getTotalReputation(skillId, customRootHash) {
        const baseEndpoint = await this.getBaseEndpoint(customRootHash);
        const skillIdString = BigNumber.from(skillId || 0).toString();
     ...
     */
    async getReputationFraction(skillId, address, customRootHash, decimalPoints = 3) {
        const { reputationAmount } = await this.getReputation(skillId, address, customRootHash);
        const { reputationAmount: totalReputation } = await this.getTotalReputation(skillId, customRootHash);
        const repAmountScaled = reputationAmount.mul(BigNumber.from(10).pow(decimalPoints));
        return (repAmountScaled.div(totalReputation).toNumber() / 10 ** decimalPoints);
    }
}
